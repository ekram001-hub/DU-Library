...
export const SeatGrid: React.FC<SeatGridProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white border border-slate-200/90 rounded-2xl p-2.5 shadow-xs">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
          ...
          <button
            ...
            title={`All Rooms (${totalCount} seats)`}
          ...
          {branchRooms.map((room) => {
            ...
            return (
              <button
                ...
                title={`${room.name} (${roomSeats.length} seats)`}
                className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-all cursor-pointer ${
              ...
              >
                ...
                <span className="font-mono">{roomSeats.length}</span>
              </button>
            );
          })}
        </div>
      ...
      {/* 3. Break Statistic (single compact pill) */}
      <div className="flex justify-end">
        <div className="inline-flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-1.5 shadow-xs">
          <Coffee className="w-4 h-4 text-emerald-600" />
          <span className="text-xs font-semibold text-emerald-800">Break</span>
          <span className="text-sm font-bold text-emerald-900 font-mono">{awayCount}</span>
        </div>
      </div>

      {/* Visual Status Legend */}
      <div className="bg-white border border-slate-200/90 rounded-xl p-2.5 flex flex-wrap items-center gap-x-4 gap-y-2 text-[11px] font-medium text-slate-700 shadow-2xs">
        <span className="font-bold text-slate-900 text-xs flex items-center gap-1.5 shrink-0">
          <span className="w-2 h-2 rounded-full bg-slate-900" /> Seat Status:
        </span>
        <div className="flex items-center gap-1.5 whitespace-nowrap">
          <span className="w-3 h-3 rounded-md bg-white border border-slate-300 shadow-2xs inline-block shrink-0" />
          <span>Available</span>
        </div>
        <div className="flex items-center gap-1.5 whitespace-nowrap">
          <span className="w-3 h-3 rounded-md bg-red-600 shadow-2xs inline-block shrink-0" />
          <span className="font-semibold text-red-700">Occupied</span>
        </div>
        <div className="flex items-center gap-1.5 whitespace-nowrap">
          <span className="w-3 h-3 rounded-md bg-emerald-600 shadow-2xs inline-block shrink-0" />
          <span className="font-semibold text-emerald-700">On Break + Timer</span>
        </div>
        <div className="flex items-center gap-1.5 whitespace-nowrap">
          <span className="w-3 h-3 rounded-md bg-blue-600 shadow-2xs inline-block shrink-0" />
          <span className="font-semibold text-blue-700">2nd Booking</span>
        </div>
        <div className="flex items-center gap-1.5 whitespace-nowrap">
          <span className="w-3 h-3 rounded-md bg-teal-800 ring-2 ring-amber-300 shadow-2xs inline-block shrink-0" />
